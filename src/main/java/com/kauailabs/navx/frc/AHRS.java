...
public class AHRS {
     ...
     */
    public AHRS(String serial_port_id, SerialDataType data_type, byte update_rate_hz) {
     ...
     * from the sensor. Note that this sensor timestamp is only provided when
     * the Register-based IO methods (SPI, I2C) are used; sensor timestamps are
     * not provided when Serial-based IO methods (TTL UART, USB) are used.
     * 
     * @return The sensor timestamp corresponding to the current AHRS sensor
     *         data.
     */
    ...
    public float getRawGyroX() {
        return this.raw_gyro_x / (DEV_UNITS_MAX / gyro_fsr_dps);
    }
    ...
    public float getRawGyroY() {
        return this.raw_gyro_y / (DEV_UNITS_MAX / gyro_fsr_dps);
    }
    ...
    public float getRawGyroZ() {
        return this.raw_gyro_z / (DEV_UNITS_MAX / gyro_fsr_dps);
    }
    ...
    public float getRawAccelX() {
        return this.raw_accel_x / (DEV_UNITS_MAX / accel_fsr_g);
    }
    ...
    public float getRawAccelY() {
        return this.raw_accel_y / (DEV_UNITS_MAX / accel_fsr_g);
    }
    ...
    public float getRawAccelZ() {
        return this.raw_accel_z / (DEV_UNITS_MAX / accel_fsr_g);
    }
    ...
    public String getFirmwareVersion() {
        double version_number = fw_ver_major;
    ...
    class IOCompleteNotification implements IIOCompleteNotification {
        ...
        public void setYawPitchRoll(YPRUpdate ypr_update, long sensor_timestamp) {
            ...
            updateListener(sensor_timestamp);
        }
        ...
        public void setAHRSPosData(AHRSPosUpdate ahrs_update, long sensor_timestamp) {
            ...
            AHRS.this.is_moving =
                    (((ahrs_update.sensor_status & AHRSProtocol.NAVX_SENSOR_STATUS_MOVING) != 0) ? true : false);
            AHRS.this.is_rotating =
                    (((ahrs_update.sensor_status & AHRSProtocol.NAVX_SENSOR_STATUS_YAW_STABLE) != 0) ? false : true);
            AHRS.this.altitude_valid =
                    (((ahrs_update.sensor_status & AHRSProtocol.NAVX_SENSOR_STATUS_ALTITUDE_VALID) != 0) ? true
                            : false);
            AHRS.this.is_magnetometer_calibrated =
                    (((ahrs_update.cal_status & AHRSProtocol.NAVX_CAL_STATUS_MAG_CAL_COMPLETE) != 0) ? true : false);
            AHRS.this.magnetic_disturbance =
                    (((ahrs_update.sensor_status & AHRSProtocol.NAVX_SENSOR_STATUS_MAG_DISTURBANCE) != 0) ? true
                            : false);
    ...
    private void updateListener(long timestamp) {
        if (updateListener != null) {
            updateListener.updated(timestamp);
        }
    }
