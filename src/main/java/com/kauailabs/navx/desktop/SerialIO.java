...
class SerialIO implements IIOProvider {
    ...
    final boolean debug =
            false; /* Set to true to enable debug output (to smart dashboard) */

    public SerialIO(String port_id, byte update_rate_hz, boolean processed_data, IIOCompleteNotification notify_sink,
            IBoardCapabilities board_capabilities) {
    ...
    protected int decodePacketHandler(byte[] received_data, int offset, int bytes_remaining) {
        ...
        int sensor_timestamp =
                0; /* Note: Serial Protocols don't provide sensor timestamps */

        if ((packet_length =
                IMUProtocol.decodeYPRUpdate(received_data, offset, bytes_remaining, ypr_update_data)) > 0) {
            notify_sink.setYawPitchRoll(ypr_update_data, sensor_timestamp);
        } else if ((packet_length =
                AHRSProtocol.decodeAHRSPosUpdate(received_data, offset, bytes_remaining, ahrspos_update_data)) > 0) {
            notify_sink.setAHRSPosData(ahrspos_update_data, sensor_timestamp);
        } else if ((packet_length =
                AHRSProtocol.decodeAHRSUpdate(received_data, offset, bytes_remaining, ahrs_update_data)) > 0) {
            notify_sink.setAHRSData(ahrs_update_data, sensor_timestamp);
        } else if ((packet_length =
                IMUProtocol.decodeGyroUpdate(received_data, offset, bytes_remaining, gyro_update_data)) > 0) {
            notify_sink.setRawData(gyro_update_data, sensor_timestamp);
        } else if ((packet_length =
                AHRSProtocol.decodeBoardIDGetResponse(received_data, offset, bytes_remaining, board_id)) > 0) {
    ...
    public void run() {
        ...
        try {
            ...
            port_reset_count++;
            if (debug) {
                SmartDashboard.putNumber("navX Port Resets", port_reset_count);
            }
        ...
        while (!stop) {
            try {
                ...
                /*
                 * If a partial packet remains from last iteration, place that
                 * at
                 */
                /*
                 * the start of the data buffer, and append any new data
                 * available
                 */
                /* at the serial port. */
                ...
                if (bytes_read > 0) {
                    ...
                    while (i < bytes_read) {
                        ...
                        if (received_data[i] != IMUProtocol.PACKET_START_CHAR) {
                            ...
                            if (debug) {
                                SmartDashboard.putNumber("navX Discarded Bytes", discarded_bytes_count);
                            }
                            continue;
                        } else {
                        ...
                        if (packet_length > 0) {
                            ...
                            if ((last_valid_packet_time - last_second_start_time) > 1.0) {
                                if (debug) {
                                    SmartDashboard.putNumber("navX Updates Per Sec", updates_in_last_second);
                                }
                            ...
                            if (packet_length > 0) {
                                ...
                                if (debug) {
                                    SmartDashboard.putNumber("navX Stream Responses", stream_response_receive_count);
                                }
                            } else {
                                ...
                                if (packet_length > 0) {
                                    ...
                                    /*
                                     * Even though a start-of-packet indicator
                                     * was found, the
                                     */
                                    /*
                                     * current index is not the start of a
                                     * packet if interest.
                                     */
                                    /*
                                    ...
                                    if (!next_packet_start_found && x == bytes_remaining) {
                                        /*
                                         * Remaining bytes don't include a
                                         * start-of-packet
                                         */
                                    ...
                                    if (discard_remainder) {
                                        ...
                                        if (!next_packet_start_found) {
                                            /*
                                             * This occurs when packets are
                                             * received that are not decoded.
                                             */
                                            /*
                                             * Bump over this packet and prepare
                                             * for the next.
                                             */
                                            if ((bytes_remaining > 2) && (received_data[i
                                                    + 1] == AHRSProtocol.BINARY_PACKET_INDICATOR_CHAR)) {
                                                /*
                                                 * Binary packet received; next
                                                 * byte is packet length-2
                                                 */
                                                ...
                                                /*
                                                 * Scan up to and including next
                                                 * end-of-packet character
                                                 * sequence, or the beginning of
                                                 * a new packet.
                                                 */
                                                for (x = 0; x < bytes_remaining; x++) {
                                                    ...
                                                    /*
                                                     * If a new start-of-packet
                                                     * is found, discard the
                                                     * ascii packet bytes that
                                                     * precede it.
                                                     */
                ...
                // This exception typically indicates a Timeout, but can also be
                // a buffer overrun error.
                stream_response_received = false;
                timeout_count++;
                if (debug) {
                    SmartDashboard.putNumber("navX Serial Port Timeout / Buffer Overrun", timeout_count);
    ...
    /**
     * Indicates whether the navX MXP is currently connected to the host
     * computer. A connection is considered established whenever a value update
     * packet has been received from the navX MXP within the last second.
     * 
    ...
    /**
     * Returns the count in bytes of data received from the navX MXP. This could
     ...
     * the navX MXP. This count should increase at the same rate indicated by
